...
impl<
		...
		ItemConfig: Default,
	> NonFungiblesV2MutateAdapter<Assets, Matcher, AccountIdConverter, AccountId, CheckAsset, CheckingAccount, ItemConfig>
{
...
impl<
		...
		ItemConfig: Default,
	> TransactAsset
	for NonFungiblesV2MutateAdapter<Assets, Matcher, AccountIdConverter, AccountId, CheckAsset, CheckingAccount, ItemConfig>
{
	...
	fn deposit_asset(what: &MultiAsset, who: &MultiLocation, context: &XcmContext) -> XcmResult {
		...
		// Check we handle this asset.
		let (class, instance) = Matcher::matches_nonfungibles(what)?;
		let who = AccountIdConverter::convert_location(who).ok_or(MatchError::AccountIdConversionFailed)?;
	...
	fn withdraw_asset(
		...
		let who = AccountIdConverter::convert_location(who).ok_or(MatchError::AccountIdConversionFailed)?;
		let (class, instance) = Matcher::matches_nonfungibles(what)?;
		Assets::burn(&class, &instance, Some(&who)).map_err(|e| XcmError::FailedToTransactAsset(e.into()))?;
		Ok(what.clone().into())
	}
}
...
///
/// This adapter combines the functionalities of both the
/// [`NonFungiblesV2TransferAdapter`] and [`NonFungiblesV2MutateAdapter`]
/// adapters, allowing handling NFTs in various scenarios.
...
/// which is useful for describing collections that do not follow an incremental
/// pattern.
